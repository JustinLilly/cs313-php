<!doctype html>
<html lang="en_us">
    <?php include $_SERVER ['DOCUMENT_ROOT'] . '/cs313-php/web/projectOne/modules/head.php'; ?>
<body>
  <main>

    <?php include $_SERVER ['DOCUMENT_ROOT'] . '/cs313-php/web/projectOne/modules/header.php'; ?>
    
    <?php include $_SERVER ['DOCUMENT_ROOT'] . '/cs313-php/web/projectOne/modules/nav.php'; ?>

  </main>
  <?php include $_SERVER ['DOCUMENT_ROOT'] . '/cs313-php/web/projectOne/modules/footer.php'; ?>
</body>
</html>